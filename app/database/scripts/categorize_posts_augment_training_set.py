...
# Categorizes 734 tumblr posts (all of them except the latest 2).
...
def extract_feature_vectors(article_list, word_list):
    ...
    for tweet in article_list:  # for each tweet
        for word in extract_words(tweet):  # for each word in the tweet
            if (len(word) > 2):
                # if it's in there set it to 1
                feature_matrix[tweet_num][word_list[word]] = 1
        tweet_num += 1
    pass
    ### ========== TODO : END ========== ###
...
y_test = OneVsRestClassifier(LinearSVC(random_state=0)).fit(
    X_train, new_y_train).predict(X_test)
...
# "UCLA Radio Comedy --> 7 Test how many of these there are. Merge with entertainment?
# "Show of the Month" or "meet the dj" --> 8
category_list = ["Invalid Tag", "Show Review", "Music Review",
                 "Interview", "Sports", "News", "Entertainment", "Comedy", "UCLA Radio"]
for article in range(len(data_new_test)):
