function calculateDateAgo(secAgo) {
  ...
  if (secAgo >= (agoRange = 60 * 60 * 24))
    agoString = `${(agoScaled = Math.floor(secAgo / agoRange))} ${agoScaled > 1
      ? 'days'
      : 'day'} ago`;
    ...
    agoString = `${(agoScaled = Math.floor(secAgo / agoRange))} ${agoScaled > 1
      ? 'minutes'
      : 'minute'} ago`;
  else if (secAgo >= -60) agoString = 'listening just now';
  else agoString = 'soon ;)';
  return agoString;
}
...
const latestTrackInfo = {
  name: null,
  artist: null,
};

function updateRecentTracks(JSONdata) {
  const oTracks = new Array().concat(JSONdata.recenttracks.track);

  const div = document.getElementById('c_data');
  let pic =
    oTracks[0].image[1]['#text'] != '' ? oTracks[0].image[2]['#text'] : null;
  const latestTrackTitle = truncateName(oTracks[0].name, 22);
  const latestTrackArtist = truncateName(oTracks[0].artist['#text'], 22);

  if (
    latestTrackTitle !== latestTrackInfo.name &&
    latestTrackArtist !== latestTrackInfo.artist
  ) {
    latestTrackInfo.name = latestTrackTitle;
    latestTrackInfo.artist = latestTrackArtist;

    const rm_currently_playing = document.getElementById('current_track');
    ...
    $('#c_data').slick(
      'slickAdd',
      `${'<div> <br />' + '<img class= "album_artwork" src="'}${pic}"</>` +
        `<strong>"${latestTrackTitle}"</strong><br />${latestTrackArtist}<img id="current_track" src="img/musicbaryt3.gif" /></div>`,
      0,
      true
    );
  }
...
function lfmRecentTracks(JSONdata) {
  ...
  for (let i = 0; i < oTracks.length; i++) {
    ...
    let pic =
      oTracks[i].image[1]['#text'] != '' ? oTracks[i].image[2]['#text'] : null;
    const track_title = truncateName(oTracks[i].name, 22);
    const artist = truncateName(oTracks[i].artist['#text'], 22);

    pic = pic == null ? 'img/no_album_artwork.jpg' : pic;
    currently_indicator = !oTracks[i].date
      ? '<img id="current_track" src="img/musicbaryt3.gif" />'
      : '';
    $('#c_data').slick(
      'slickAdd',
      `${'<div> <br />' + '<img class= "album_artwork" src="'}${pic}"</>` +
        `<strong>"${track_title}"</strong><br />${artist}${currently_indicator}</div>`
    );
  }
