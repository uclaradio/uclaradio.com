function calculateDateAgo(secAgo) {
  ...
  if (secAgo >= (agoRange = 60 * 60 * 24))
    agoString =
      (agoScaled = Math.floor(secAgo / agoRange)) +
      ' ' +
      (agoScaled > 1 ? 'days' : 'day') +
      ' ago';
  else if (secAgo >= (agoRange = 60 * 60))
    agoString =
      (agoScaled = Math.floor(secAgo / agoRange)) +
      ' ' +
      (agoScaled > 1 ? 'hours' : 'hour') +
      ' ago';
  else if (secAgo >= (agoRange = 60))
    agoString =
      (agoScaled = Math.floor(secAgo / agoRange)) +
      ' ' +
      (agoScaled > 1 ? 'minutes' : 'minute') +
      ' ago';
  ...
  if (
    ...
    $('#c_data').slick(
      ...
      '<div> <br />' +
        ...
        '"</strong><br />' +
        latestTrackArtist +
        '<img id="current_track" src="img/musicbaryt3.gif" /></div>',
      0,
      true
    );
  }
